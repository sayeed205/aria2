...
import { AuthenticationError, JsonRpcError, NetworkError } from "../../src/types/errors.ts";
...
describe("JsonRpcTransport", () => {
    ...
    describe("call method", () => {
        it("should make successful JSON-RPC call", async () => {
            ...
            const result = await transport.call("aria2.addUri", [["http://example.com/file.zip"]]);
        ...
        it("should not include secret token when not configured", async () => {
            ...
            await transportWithoutSecret.call("aria2.addUri", [["http://example.com/file.zip"]]);
        ...
        it("should generate unique request IDs", async () => {
            ...
            const secondRequestBody = JSON.parse(fetchCalls[1].options.body as string);
    ...
    describe("error handling", () => {
        ...
        it("should throw NetworkError on invalid JSON-RPC response", async () => {
            mockFetch(new Response(JSON.stringify({ invalid: "response" }), {
                status: 200,
                headers: { "content-type": "application/json" },
            }));
        ...
        it("should throw JsonRpcError on JSON-RPC error response", async () => {
            ...
            mockFetch(new Response(JSON.stringify(errorResponse), {
                status: 200,
                headers: { "content-type": "application/json" },
            }));
        ...
        it("should throw JsonRpcError when response missing result", async () => {
            ...
            mockFetch(new Response(JSON.stringify(invalidResponse), {
                status: 200,
                headers: { "content-type": "application/json" },
            }));
    ...
    describe("request building", () => {
        it("should build correct JSON-RPC request structure", async () => {
            ...
            assertEquals(requestBody.params, ["token:test-secret", ["gid", "status"]]);
