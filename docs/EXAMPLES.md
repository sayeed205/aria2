...
> **⚡ NOTE:**
> All JSON-RPC communication now uses **WebSocket only** (`ws://` or `wss://`).
> HTTP/HTTPS/fetch/REST is **not supported** as a transport—your aria2 must expose a WebSocket endpoint.
...
async function displayProgress(gid: string): Promise<void> {
  ...
  const progressBar =
    "█".repeat(Math.floor(progress.progress / 2)) +
  ...
  const interval = setInterval(async () => {
    try {
      ...
      for (const status of statuses) {
        const progress =
          (parseInt(status.completedLength) / parseInt(status.totalLength)) *
          100;
        const speed = parseInt(status.downloadSpeed);
        const speedMB = (speed / 1024 / 1024).toFixed(2);
        ...
        console.log(
          `${statusIcon} ${status.gid}: ${progress.toFixed(1)}% (${speedMB} MB/s)`,
        );
    ...
    try {
      ...
      if (error instanceof NetworkError) {
        ...
        switch (error.rpcCode) {
          ...
          default:
            console.error(
              `🔧 aria2 error (${error.rpcCode}): ${error.message}`,
            );
            if (attempt < retries) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      const overallProgress =
        totalSize > 0 ? (totalCompleted / totalSize) * 100 : 0;
      ...
      console.log(
        `📈 Active: ${activeCount}, Complete: ${completeCount}, Errors: ${errorCount}`,
      );
...
class DownloadQueue {
    ...
    while (this.queue.length > 0 || this.active.size > 0) {
      ...
      if (activeGids.length > 0) {
        const statuses = await Promise.allSettled(
          activeGids.map((gid) =>
            this.aria2.tellStatus(gid, ["gid", "status"]),
          ),
        );
  ...
  async getStatus(): Promise<void> {
    console.log(
      `Queue: ${this.queue.length} waiting, ${this.active.size} active`,
    );
  ...
  private async monitorDownloads(gids: string[]): Promise<void> {
    ...
    const interval = setInterval(async () => {
      try {
        const statuses = await Promise.all(
          gids.map((gid) =>
            this.aria2.tellStatus(gid, ["gid", "status", "files"]),
          ),
        );
        ...
        console.log(
          `📊 Progress: ${completed}/${gids.length} complete, ${active} active, ${errors} errors`,
        );
        ...
        if (completed + errors === gids.length) {
          ...
          for (const status of statuses) {
            const filename =
              status.files[0]?.path.split("/").pop() || status.gid;
  ...
  async performBackup(config: BackupConfig): Promise<void> {
    ...
    for (const source of config.sources) {
      try {
        ...
        console.error(
          `❌ Failed to queue backup for ${source}:`,
          error.message,
        );
      }
    }
  ...
  private async monitorBackup(
    ...
    const interval = setInterval(async () => {
      try {
        ...
        for (const status of statuses) {
          ...
          switch (status.status) {
            ...
            case "error":
              errorCount++;
              console.error(
                `❌ Backup failed: ${status.files[0]?.path} - ${status.errorMessage}`,
              );
              break;
          }
        }
        ...
        console.log(
          `📊 Backup Progress: ${progress.toFixed(1)}% (${runtime}s)`,
        );
        console.log(
          `📈 Active: ${activeCount}, Complete: ${completeCount}, Errors: ${errorCount}`,
        );
